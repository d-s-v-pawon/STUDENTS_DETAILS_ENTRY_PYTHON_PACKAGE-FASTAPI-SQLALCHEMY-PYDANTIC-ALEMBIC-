...
from sqlalchemy import create_engine,Column,Integer,String,BIGINT,Float,select
...
def enter_student(STU : StudentDetails , session : Session = Depends(get_db)):
    student = STUDENT(NAMES=STU.NAMES,
                            GENDERS=STU.GENDERS,
                            AGES=STU.AGES,
...
def get_students(skip : int = 0 , limit : int = 100 ,session : Session = Depends(get_db)):
    #students_list = session.query(STUDENT).order_by(STUDENT.ROLL_NUMBERS).offset(skip).limit(limit).all()
    # stmt = select(STUDENT).order_by(STUDENT.CONTACT_NUMBERS).offset(skip).limit(limit)
    students_list = session.execute(select(STUDENT).order_by(STUDENT.CONTACT_NUMBERS).offset(skip).limit(limit)).scalars().all()
